...
Diffbot.prototype.sendBulk = function (options, callback) {
  ...
  if (!options.uri || !options.name || !options.webhookURL) {
    throw new Error("the Name, webhook, & URI is required.");
  }

  var diffbot_url = "https://api.diffbot.com/v3/bulk?token=" + this.token + "&name=" + options.name + "&urls=" + encodeURIComponent(options.uri) + 
  "&apiUrl=https://api.diffbot.com/v3/article?fields=quotes,sentiment" + '&notifyWebhook=' + options.webhookURL + "&format=json";
